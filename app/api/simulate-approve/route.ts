...
export async function POST(req: Request) {
  try {
    // Session email (or allow explicit override in body)
    ...
    if (!email) {
      return NextResponse.json({ success: false, error: "Unauthorized (no session/email)" }, { status: 401 });
    }

    const me = await prisma.user.findUnique({ where: { email }, select: { id: true } });
    if (!me) return NextResponse.json({ success: false, error: "User not found" }, { status: 404 });

    // Pick commission: supplied id or latest PENDING for this user
    ...
    if (!targetId) {
      ...
      if (!latest) {
        return NextResponse.json({ success: false, error: "No pending commissions for this user" }, { status: 400 });
      }
      targetId = latest.id;
    }

    // Ensure it belongs to this user
    const own = await prisma.commission.findFirst({
      where: { id: targetId, userId: me.id },
      select: { id: true, status: true },
    });
    if (!own) {
      return NextResponse.json({ success: false, error: "Commission not found for this user" }, { status: 404 });
    }
    ...
    console.error("simulate-approve error:", err?.message ?? err);
