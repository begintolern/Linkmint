...
export default function CreateLinkPage() {
  ...
  const handleGenerate = () => {
    if (!url.trim()) {
      alert("Please enter a valid URL before generating a link.");
      return;
    }

    // ✅ Save demo link to localStorage
    const newLink = {
    ...
    try {
      ...
      const updated = [newLink, ...parsed].slice(0, 10); // keep max 10
      localStorage.setItem("recentLinks", JSON.stringify(updated));
    } catch {
      // ignore storage errors
    }

    // ✅ Redirect back to Smart Links
