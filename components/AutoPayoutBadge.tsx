...
export default function AutoPayoutBadge() {
  const on = isAutoPayoutEnabled();
  const disbursing = isAutoDisburseEnabled();

  const cls =
    "inline-flex items-center gap-1 rounded-full px-2 py-1 text-[11px] font-semibold";
  const colorOn = on ? "bg-emerald-100 text-emerald-700 ring-1 ring-emerald-200"
  ...
  return (
    <span className={`${cls} ${colorOn}`} title={`Disburse: ${disbursing ? "ON" : "OFF"}`}>
      ...
      {on && (
        <span className={`ml-1 rounded-sm px-1 py-[1px] text-[10px] ${disbursing
