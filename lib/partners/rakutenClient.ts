...
// Stable approach: use LinkShare (linksynergy.com) for BOTH tokens and APIs to avoid TLS CN issues.

const LS_BASE = "https://api.linksynergy.com";           // Partnerships, Product Search, Deep Links, Advertisers
const LS_TOKEN_URL = "https://api.linksynergy.com/token"; // Mint tokens from the same host

type OAuthToken = { access_token: string; token_type: "Bearer"; expires_in: number };

// separate cache for LS token
let cachedTokenLS: { token?: string; expiresAt?: number } = {};

/* ---------------- Token fetcher (LinkShare) ---------------- */
...
/* ---------------- Client helpers (all via LinkShare host) ---------------- */

// v2 advertisers catalog — served on LS as well; partnerships payloads link to /v2/advertisers on same host
export async function listAdvertisers(params: { page?: number; pageSize?: number; q?: string } = {}) {
  return lsGET<{ data?: unknown[]; total?: number }>(`/v2/advertisers`, {
    page: params.page ?? 1,
...
// legacy advertiserSearch (returns midlist)
...
// partnerships (approved/pending/declined)
